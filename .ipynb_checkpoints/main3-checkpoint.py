...
class MotionFeatureExtractor(nn.Module):
    def __init__(self, input_dim, motion_dim):
        ...
        self.fc = nn.Linear(input_dim, motion_dim)  # Simplified for demonstration
...
class DSNWithMotion(nn.Module):
    ...
    def forward(self, x):
        # No reshaping of x needed before this point
        motion_features = self.motion_extractor(x)  # Directly obtain motion features
        
        # Now x and motion_features should have compatible shapes for concatenation
...
# Motion feature extraction options
parser.add_argument('--motion-dim', type=int, default=64, help="dimension of motion features (default: 64)")
...
def main():
    ...
    model = DSNWithMotion(in_dim=args.input_dim, motion_dim=args.motion_dim, hid_dim=args.hidden_dim, num_layers=args.num_layers, cell=args.rnn_cell)
