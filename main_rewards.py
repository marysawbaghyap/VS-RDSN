...
def main():
    ...
    for epoch in range(start_epoch, args.max_epoch):
        ...
        for idx in idxs:
            ...
            saliency_map = dataset[key]['saliency_map'][...] # saliency map for the current video
            seq = torch.from_numpy(seq).unsqueeze(0) # input shape (1, seq_len, dim)
            if use_gpu: 
                seq = seq.cuda()
                saliency_map = saliency_map.cuda() # If using GPU, move saliency_map to GPU as well
            ...
            for _ in range(args.num_episode):
                ...
                reward = compute_reward(seq, saliency_map, actions, use_gpu=use_gpu) # Pass saliency_map to compute_reward function
